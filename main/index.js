...
app.post("/db/seed", async (_, res) => {
  db.any(
    "CREATE TABLE test_table(id SERIAL PRIMARY KEY, key VARCHAR NOT NULL UNIQUE, value VARCHAR);"
  )
    .then((_) => {
...
app.post("/db/upsert", function (req, res) {
  for (const [key, value] of Object.entries(req.body)) {
    db.any(
      "INSERT INTO test_table (key, value) VALUES($1, $2) ON CONFLICT (key) DO UPDATE SET value = $2",
      [key, value]
    ).catch((e) => {
